...
import { ShoppingCart, Loader2, Users, DollarSign, Wallet, RefreshCw, Tag, Trash2 } from "lucide-react";
...
type TransactionState = 'idle' | 'listing' | 'purchasing' | 'withdrawing' | 'success' | 'error'
...
export default function Marketplace() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (activeTab === 'buy') {
        ...
        const availableListings = listings.filter(listing => listing.seller !== user.addr);
        setMarketplaceListings(availableListings);
        console.log(`📊 Loaded ${availableListings.length} marketplace listings`);
  ...
  const handlePurchaseBond = async (listing: MarketplaceListing) => {
    if (!confirm(`Purchase Bond #${listing.bondID} for ${listing.price} FLOW from ${listing.seller}?`)) {
      return;
    }
    ...
    try {
      console.log(`🛒 Purchasing bond ${listing.bondID} for ${listing.price} FLOW...`);
      ...
      setTxStatus({
        state: 'error',
        statusString: error instanceof Error ? `❌ Failed to purchase bond: ${error.message}` : '❌ Failed to purchase bond',
        txId: null
      });
  ...
  const handleWithdrawBond = async (bondId: number) => {
    ...
    try {
      ...
      const result = await marketplaceService.withdrawBondFromSale(bondId.toString());
      ...
      setTxStatus({
        state: 'error',
        statusString: error instanceof Error ? `❌ Failed to withdraw bond: ${error.message}` : '❌ Failed to withdraw bond',
        txId: null
      });
      ...
      >
        <Card className="card-professional mx-auto max-w-md">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-2xl font-bold mb-4 gradient-text">Connect Wallet to Access Marketplace</h3>
    ...
    <div className="app-container space-y-8">
        ...
        <Card className="card-professional">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl font-bold gradient-text">Bond Marketplace</CardTitle>
                <CardDescription>List your bonds for sale or purchase bonds from other users</CardDescription>
              </div>
            </div>
      ...
      >
        <div className="flex space-x-1 bg-muted/30 p-1 rounded-lg">
          {[
            { key: 'buy', label: '🛒 Buy Bonds', desc: 'Browse available bonds' },
            { key: 'sell', label: '💰 Sell Bonds', desc: 'List your bonds for sale' },
            { key: 'manage', label: '📊 Manage Listings', desc: 'Manage your active listings' }
          ].map(tab => (
            <button
              key={tab.key}
      ...
      {txStatus.statusString && (
        <motion.div
          ...
          className={`p-4 rounded-lg border ${
            txStatus.state === 'success' ? 'bg-success/10 text-success border-success/20' :
          ...
          <Card className="card-professional">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-semibold">🛒 Available Bonds</CardTitle>
                  <CardDescription>Browse and purchase bonds listed by other users</CardDescription>
                </div>
                <Button 
                  onClick={loadData}
                  variant="outline"
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No bonds available</h3>
                  <p className="text-muted-foreground mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {marketplaceListings.map((listing) => (
                    ...
                    >
                      <h3 className="font-semibold text-lg mb-2">🔗 Bond #{listing.bondID}</h3>
                      <div className="space-y-1 text-sm text-muted-foreground mb-3">
                        <p><span className="font-medium">💰 Price:</span> {listing.price} FLOW</p>
                        <p><span className="font-medium">👤 Seller:</span> 
                          <span className="font-mono text-xs ml-1">
                            {formatAddress(listing.seller)}
                          </span>
                        </p>
                        <p className={`font-medium ${listing.isAvailable ? 'text-success' : 'text-error'}`}>
                          {listing.isAvailable ? '✅ Available' : '❌ Sold'}
                        </p>
                      </div>
                      <Button
                        onClick={() => handlePurchaseBond(listing)}
                        disabled={txStatus.state === 'purchasing' || !listing.isAvailable}
                        className="w-full btn-primary"
                        size="sm"
                      >
                        <ShoppingCart className="w-3 h-3 mr-1" />
                        {txStatus.state === 'purchasing' ? "Processing..." : `Buy for ${listing.price} FLOW`}
                      </Button>
                    </motion.div>
                  ))}
                </div>
        ...
        >
          <Card className="card-professional">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-semibold">💰 List Your Bonds for Sale</CardTitle>
                  <CardDescription>List your bonds for sale on the marketplace</CardDescription>
                </div>
                <Button 
                  onClick={loadData}
                  variant="outline"
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No bonds to list</h3>
                  <p className="text-muted-foreground mb-6">
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg mb-2">🔗 Bond #{bond.id}</h3>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-muted-foreground">
                            <p><span className="font-medium">💰 Principal:</span> {bond.principal} FLOW</p>
                            <p><span className="font-medium">📈 Yield:</span> {(bond.yieldRate * 100).toFixed(1)}%</p>
                        ...
                        <div className="ml-4 flex items-center space-x-2">
                          ...
                          <Button
                            onClick={() => handleListBond(bond.id, listingPrices[bond.id])}
                            disabled={txStatus.state === 'listing' || !listingPrices[bond.id] || isMatured(bond.maturityDate)}
                            variant="default"
                            size="sm"
                            className="h-8 btn-primary"
                          >
                            <Tag className="w-3 h-3 mr-1" />
                            {isMatured(bond.maturityDate) ? "Matured" : "List for Sale"}
                          </Button>
                        </div>
                      </div>
                    </motion.div>
        ...
        >
          <Card className="card-professional">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-semibold">📊 Your Active Listings</CardTitle>
                  <CardDescription>Manage your bonds listed for sale</CardDescription>
                </div>
                <Button 
                  onClick={loadData}
                  variant="outline"
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm text-muted-foreground">
                            <p><span className="font-medium">💰 Listed Price:</span> 
                              <span className="text-primary font-bold ml-1">{listing.price} FLOW</span>
                            </p>
                            <p><span className="font-medium">📈 Status:</span> 
                              <span className={listing.isAvailable ? "text-success" : "text-error"}>
                                {listing.isAvailable ? " ✅ Available" : " ❌ Sold"}
                              </span>
                            </p>
                          </div>

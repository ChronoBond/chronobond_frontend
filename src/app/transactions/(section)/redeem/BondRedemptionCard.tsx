...
interface BondRedemptionCardExtendedProps extends BondRedemptionCardProps {
  onReinvest?: (bond: BondDetails) => void;
}
...
export const BondRedemptionCard = ({
  ...
  onReinvest,
  isRedeeming,
}: BondRedemptionCardExtendedProps) => {
  return (
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button
            ...
            className="bg-brand-accent text-white hover:bg-brand-accent/90 shadow-lg"
            size="lg"
          >
            {isRedeeming ? (
              <>
          ...
          <Button
            onClick={() => onReinvest?.(bond as BondDetails)}
            disabled={isRedeeming}
            variant="outline"
            className="border-white/20 text-semantic-text hover:bg-white/10 hover:border-white/40"
            size="lg"
          >
            <RotateCcw className="w-4 h-4" /> Reinvest
          </Button>
        </div>
      </div>
    </div>

...
export const HoldingsRedeemModal = ({
  ...
  const [status, setStatus] = React.useState<'idle' | 'confirming' | 'success' | 'error'>('idle');
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Failed to redeem bond');
      setStatus('error');
      setTimeout(() => {
        setStatus('idle');
      }, 3000);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} isDisabled={isRedeeming || status === 'confirming'}>
      <div 
      ...
      >
        ...
        <div className="space-y-3 mb-6">
          <div className="bg-semantic-overlay rounded-lg p-4 border border-semantic-border">
            <div className="flex justify-between items-center mb-3">
              <span className="text-xs font-semibold text-semantic-text/80 uppercase tracking-wide">Principal</span>
              <span className="text-lg font-bold text-semantic-text">{formatFlow(bond.principal)}</span>
            </div>
            <div className="flex justify-between items-center mb-3">
              <span className="text-xs font-semibold text-semantic-text/80 uppercase tracking-wide">Expected Yield</span>
              <span className="text-lg font-bold text-brand-accent">+{formatFlow(expectedYield)}</span>
            </div>
            <div className="border-t border-semantic-border pt-3 flex justify-between items-center">
              <span className="text-xs font-semibold text-semantic-text uppercase tracking-wide">Total Redemption</span>
              <span className="text-2xl font-bold text-brand-accent">{formatFlow(totalValue)}</span>
            </div>
          </div>
        </div>
        ...
        {status === 'success' && (
          <div className="mb-6 p-4 rounded-lg bg-brand-accent/10 border border-brand-accent/30 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-brand-accent">Bond redeemed successfully!</p>
              <p className="text-xs text-brand-accent/70">Your funds will be transferred to your wallet</p>
            </div>
          </div>
        )}
        ...
        {status === 'confirming' && (
          <div className="mb-6 p-4 rounded-lg bg-brand-primary/10 border border-brand-primary/30 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-brand-primary">Processing redemption...</p>
              <p className="text-xs text-brand-primary/70">Please approve the transaction in your wallet</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          <Button
            variant="outline"
            onClick={onClose}
            disabled={isRedeeming || status === 'confirming' || status === 'success'}
            className="flex-1"
          >
          ...
          <Button
            ...
            disabled={isRedeeming || status === 'confirming' || status === 'success' || status === 'error'}
            className="flex-1 gap-2"
          >
            {status === 'confirming' && <Loader2 className="w-4 h-4 animate-spin" />}
            {status === 'success' ? 'Redeemed' : status === 'confirming' ? 'Redeeming...' : 'Confirm Redemption'}
          </Button>
        </div>
